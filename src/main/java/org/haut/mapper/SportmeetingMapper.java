...
public interface SportmeetingMapper {
    ...
    @Results(id = "sportmeetingMap", value = {
            ...
            @Result(column = "s_id", property = "broadcast", many = @Many(select = "org.haut.mapper.BroadcastMapper.selectBybSportmeetingid", fetchType = FetchType.LAZY))
    })
    ...
    @Insert("insert into sportmeeting(s_id,s_name,s_theme,s_place,s_date,s_startTime,s_endTime,s_description,s_status) value (#{sId} ,#{sName},#{sTheme} ,#{sPlace} ,#{sDateStr} ,#{sStarttimeStr} ,#{sEndtimeStr} ,#{sDescription} ,#{sStatus}  )")
