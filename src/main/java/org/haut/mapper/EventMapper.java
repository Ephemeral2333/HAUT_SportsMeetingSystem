...
public interface EventMapper {
    ...
    @Results(id = "eventMap", value = {
            ...
            @Result(column = "e_sportmeetingid", property = "sportmeeting", one = @One(select = "org.haut.mapper.SportmeetingMapper.selectByPK", fetchType = FetchType.EAGER)),
    })
    ...
    @Results(id = "eventMap2", value = {
            ...
            @Result(column = "e_unit", property = "eUnit"),
            @Result(column = "e_sportmeetingid", property = "sportmeeting", one = @One(select = "org.haut.mapper.SportmeetingMapper.selectByPK", fetchType = FetchType.EAGER)),
            @Result(column = "e_judgementid", property = "user", one = @One(select = "org.haut.mapper.UserMapper.selectByPK", fetchType = FetchType.EAGER)),
    })
    List<Event> findAll();
    ...
    @Select("select distinct e.* from event e right join matches m on e.e_id = m.m_eid where isnull(m.m_score) and e.e_judgementid = #{sId}")
    @Results(value = {
            @Result(id = true, column = "e_id", property = "eId"),
            @Result(column = "e_name", property = "eName"),
            @Result(column = "e_gender", property = "eGender"),
    ...
    List<Event> selectNoScore(String sId);
