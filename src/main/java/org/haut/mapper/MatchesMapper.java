...
public interface MatchesMapper {
    ...
    @Results(id = "matchesMap2", value = {
            ...
            @Result(column = "m_eid", property = "event", one = @One(select = "org.haut.mapper.EventMapper.selectByPK", fetchType = FetchType.EAGER)),
    })
    ...
    @Results(id = "matchesMap", value = {
            ...
            @Result(column = "m_rank", property = "mRank"),
            @Result(column = "m_eid", property = "event", one = @One(select = "org.haut.mapper.EventMapper.selectByPK", fetchType = FetchType.EAGER)),
            @Result(column = "m_no", property = "user", one = @One(select = "org.haut.mapper.UserMapper.selectByPKToS", fetchType = FetchType.LAZY))
    })
    ...
    @Insert("insert into matches(m_no,m_eid,m_sort) values (#{mNo} ,#{mEid} ,#{mSort}) ")
    void insert(Matches matches);

    @Update("update matches set m_no = #{mNo} ,m_eid=#{mEid} ,m_sort=#{mSort} where m_id = #{mId}")
    ...
    @Update("update matches set m_score = #{mScore}, m_rank = #{mRank} where m_id = #{mId} ")
