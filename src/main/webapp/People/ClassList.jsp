                                    ...
                                    <tr style="text-align: center">
                                        ...
                                        <td class="text-center">
                                            <button type="button" class="btn bg-olive btn-xs "
                                                    onclick="location.href='${pageContext.request.contextPath}/Detail/ClassesDetail.jsp?cId=${classes.CId}'">
                                                详情
                                            </button>
...
<script>
    ...
    function findObjects() {
        ...
        var url = "${pageContext.request.contextPath}/department/findAllDepts";
        $.get(url, function (data) {
            var html = "";
            for (var i = 0; i < data.length; i++) {
    ...
    function getClasses(id, ctid, cdid) {
        ...
        var url2 = "${pageContext.request.contextPath}/department/findAllDepts";
        ...
        var url3 = "${pageContext.request.contextPath}/classes/findById?id=" + id;
    ...
    $("#Submitbtn").click(function () {
        ...
        if (updateorinsert == "update") {
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/classes/update", //提交的地址
                async: false,
                data: {
                    "cId": $("#CId").val(),
                    "cName": $("#CName").val(),
                    "cMajor": $("#CMajor").val(),
                    "cHeadmasterid": $("#CHeadmasterid").val(),
                    "cDeptid": $("#CDeptid").val()
                },
                dataType: "json",
                error: function () {  //失败的话
            ...
            $.ajax({
                type: "POST",   //提交的方法
                url: "${pageContext.request.contextPath}/classes/add", //提交的地址
                async: false,
                data: {
                    "cName": $("#CName").val(),
                    "cMajor": $("#CMajor").val(),
    ...
    function deleteClassesById(cId) {
        if (confirm("您确定要删除吗？")) {
            location.href = '${pageContext.request.contextPath}/classes/deleteByPK?cId=' + cId;
            alert("删除成功!");
            location.href = '${pageContext.request.contextPath}/People/ClassList.jsp';
        }
    }
